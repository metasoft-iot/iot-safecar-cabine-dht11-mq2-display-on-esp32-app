...
bool EdgeServiceClient::sendTelemetry(float temperature, float humidity, 
                                      int gasValue, double latitude, double longitude) {
    if (!isConnected()) {
    ...
    // Create JSON payload
    JsonDocument doc;
    doc["sensor_location"] = "CABINA";
    doc["cabin_temperature_celsius"] = temperature;
    doc["cabin_humidity_percent"] = humidity;
    
    // MQ2 Gas Sensor mapping
    // Value > 1000 indicates gas detection
    if (gasValue > 1000) {
        doc["gas_type"] = "methane";  // Default to methane for detection
        doc["gas_concentration_ppm"] = (float)gasValue;
    } else {
        // Low background levels
        doc["gas_type"] = "methane";
        doc["gas_concentration_ppm"] = (float)gasValue;
    }
    
    // GPS Location
    // If GPS has no fix, latitude/longitude will be 0.0
    // Edge service will handle this gracefully
